export const languageChinese = {
	...
	"help": {
	  ...
	  "oaiapikey": "OpenAI的API密钥。您可以在https://platform.openai.com/account/api-keys 处获取。",
	  ...
	  "forceUrl": "如果不为空，请求将会被发送到您输入的URL。",
	  ...
	  "emotion": "激活情绪图像功能会使系统根据角色的情绪显示图像，该情绪是通过分析角色的回应得出的。您必须输入情绪名称作为单词 *(如 joy，happy，fear 等)*。如果存在，以**neutral**命名的情绪将成为默认情绪。必须有3个以上的图像才能正常工作。",
	  "imggen": "图像生成功能会从外部程序生成并显示图像。该图像是通过分析当前聊天生成的图像提示词生成的。\n\n图像生成功能是基于键值参数进行分析的，这些参数可以在下面配置。\n\n**'always'** 键总是应用，并且不会改变。**'negative'** 键对于图像生成始终应用负值。\n\n其他键的值将根据随着聊天的进行而改变。\n\n如果一个键的名字前面有特殊字符，它就有特殊的效果。\n- 如果键以 **|** 开始，键的值将不会改变。\n- 如果键以 **$** 开始，键的值更有可能改变。\n\n当图像首次生成时，您只能通过修改下面的'当前图像生成数据'来改变它。",
	  ...
	  "oogaboogaURL": "如果您的WebUI支持旧版本的api，您的url应该像*https:.../run/textgen*\n\n如果您的WebUI支持新版api，您的url应该像*https://.../api/v1/generate* 使用api服务器作为主机，并将--api添加到参数中。",
	  ...
	  "additionalAssets": "在您的聊天中显示的额外资源。\n\n - 使用`{{raw::<asset name>}}`作为路径\n - 使用`{{image::<asset name>}}`作为图片\n - 使用`{{video::<asset name>}}`作为视频\n - 使用`{{audio::<asset name>}}` 作为音频\n    - 建议放在背景 HTML中",
	  "superMemory": "超级记忆通过给 AI 提供总结数据使您的角色记忆更多信息\n\n超级记忆模型是一个总结文本的模型。推荐使用Davinci，除非是具有超过2000个token数的高度总结能力的未经过滤模型，否则不推荐使用辅助模型。\n\n超级记忆提示决定了应发送什么提示词进行总结。如果您留空，它将使用默认提示词。建议留空。\n\n在所有设置都完成后，您可以在角色的设置中启用它。",
	  "replaceGlobalNote": "如果不为空，则将当前全局注释替换为此。",
	  "backgroundHTML": "将被注入聊天荧幕背景的Markdown/HTML数据。\n\n您也可以使用其他资源。例如，您可以使用`{{audio::<asset name}}`添加背景音乐\n\n此外，您还可以将这些数据与其他资源一起使用：\n - `{{bg::<asset name>}}`: 将背景注入资源",
	  "additionalText": "当AI认为需要时，将添加到角色描述中的文本，因此您可以在这里放置长文本。用双换行符分隔。",
	  "charjs": "角色JS是一个会与角色一起运行的Javascript代码。例如，您可以查看`https://github.com/kwaroran/RisuAI/blob/main/src/etc/example-char.js` 目前不建议使用，因为安全原因。导出将不包括此项。",
	  ...
	  "genTimes": "此选项用于设置在支持模型上生成响应的次数。除了第一个响应外，其他响应将作为缓存重新生成。这可以降低模型的成本，但如果您在没有重新生成的情况下使用它，也可以增加成本。",
	  ...
	  "additionalParams": "将添加到请求体的其他参数。如果您想排除一些参数，您可以将`{{none}}`放在值中。如果您想添加标头而不是体，您可以将`header::`放在键的前面，如`header::Authorization`。如果您想将值作为json，您可以将`json::`放在值的前面，如`json::{\"key\":\"value\"}`。否则，值的类型将自动确定。",
	...
	"setup": {
	  ...
	  "texttheme": "选择您的文字颜色",
	  "inputName": "最后，输入您的昵称。",
	  "welcome": "欢迎来到RisuAI! 在这里，我将指导您进行设置。首先，我该如何称呼您？",
	  "welcome2": "您好{username}! 在我们开始之前，我会问您一些问题。您可以稍后在设置中更改这些设置。\n\n首先选择AI提供商。",
	...
	"advancedSettingsWarn": "警告：如果您不知道该选项的作用，请不要更改它！",
	...
	"removeConfirm": "您真的要删除： ",
	"removeConfirm2": "您真的要删除： ",
	"exportConfirm": "您想要导出这个吗？",
	"insertOrder": "插入顺序",
	"activationKeys": "激活键",
	...
	"files": "文件",
	"backupConfirm": "您真的想要保存备份吗？",
	"backupLoadConfirm": "您真的想要加载备份吗？所有数据将丢失！",
	"backupLoadConfirm2": "您真的，真的想要加载备份吗？所有数据将丢失！",
	...
	"instantRemoveConfirm": "您是否想只删除一条消息？如果您选择否，那么它下面的消息也将被删除。",
	"textColor": "文本颜色",
	"classicRisu": "经典Risu",
	"highcontrast": "高对比度",
	"quickPreset": "您可以通过 Ctrl +（预设的索引）快速更改预设",
	...
	"clickToEdit": "点击文本以编辑",
	"setNodePassword": "设置您的密码以确保安全",
	"inputNodePassword": "输入您的密码。如果您不记得，请删除伺服器档案中的save/__password.txt并重新启动伺服器。",
	"simple": "简易",
	"advanced": "高级",
	"askReRollAutoSuggestions": "重新生成自动建议",
	"creatingSuggestions": "生成建议中...",
	"orderByOrder": "按顺序交谈",
	"removeFromGroup": "您真的想从群组中删除{{char}}吗？",
	"talkness": "健谈程度",
	"active": "活跃",
	...
	"googleDriveInfo": "连接到Google Drive以同步您的数据。",
	...
	"confirmRecommendedPreset": "该模型有一个推荐设置。您想将设置更改为该推荐设置吗？（您可以在易访问性设置中关闭询问）",
	...
	"resetPromptTemplateConfirm": "您确定要重置提示词模板吗？",
	"emotionMethod": "情绪方法",
	...
	"export": "导出",
	"import": "导入",
	"supporterThanks": "支持者鸣谢",
	"supporterThanksDesc": "感谢您的支持！",
	"donatorPatreonDesc": "预设情况下，不会在列表中显示，以保护隐私。如果您想显示您的昵称，请前往RisuAI的Patreon页面并按下链接按钮。",
	...
	"password": "密码",
	"license": "许可证",
	"licenseDesc": "您可以为下载者选择许可证以限制您的卡片提示的使用。",
	"passwordDesc": "您可以设置一个密码来保护您的卡片免受未经授权的访问。",
	...
	"basicInfo": "基本信息",
	"moduleContent": "模块内容",
	"confirmRemoveModuleFeature": "您真的想要移除这个功能吗？这个操作不能撤销。",
	"editModule": "编辑模块",
	"importModule": "导入模块",
	"download": "下载",
	"edit": "编辑",
	"enableGlobal": "全球启用",
	"chatModulesInfo": "您可以为这个聊天启用或禁用模块。",
	"sideMenuRerollButton": "侧菜单重新生成按钮",
	"persistentStorage": "持久存储",
	"persistentStorageSuccess": "存储成功持久化",
	"persistentStorageFail": "存储未持久化。您是否拒绝了请求？或者浏览器不支持？",
	"persistentStorageRecommended": "推荐持久存储",
	"persistentStorageDesc": "您的浏览器支持持久存储。这被推荐用于更好的性能和用户体验。",
	"enable": "启用",
	"postFile": "发布文件",
	...
	"updateRealmDesc": "您正试图将您的角色更新为 RisuRealm。这将会将您的角色更新为 RisuRealm，且无法恢复。",
