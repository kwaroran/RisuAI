export const languageChineseTraditional = {
  ...
  "errors": {
    ...
    "networkFetchWeb": "這可能是由 CORS 錯誤引起的。這種情況只會在使用網頁版時發生，因為瀏覽器存在一些限制。請嘗試使用本地版或其他版本的 RisuAI。",
  ...
  "help": {
    ...
    "antiClaudeOverload": "若 Claude 過載發生，RisuAI 會透過繼續相同的提示來阻止它，以減少過載的機率。此功能僅適用於流式回應（Streamed Responses），對非官方 API 端點可能無效。",
    ...
    "defaultVariables": "可在此處設定自訂預設變數。使用 `<variable name>=<variable value>` 格式，每行一個。例如：`name=RisuAI`，可在觸發式和 CBS 變數中使用，如：`{{getvar::A}}`、`{{setvar::A::B}}` 或 `{{? $A + 1}}`。若提示詞範本的預設變數與角色的預設變數名稱相同，系統將使用角色的預設變數。",
  ...
  "setup": {
    ...
    "welcome": "歡迎使用 RisuAI！我會引導你進行設定。請問我該如何稱呼你？",
  ...
  "donatorPatreonDesc": "為保護隱私，默認不會顯示在名單中。若想顯示您的暱稱，請前往 RisuAI 的 Patreon 頁面並點擊連結按鈕。",
  ...
  "realmDesc": "RisuRealm 是 RisuAI 的內容分享平台，你可以將角色分享給其他使用者。",
  ...
  "shareExport": "分享/匯出",
  "risupresetDesc": "Risupreset 是專為 RisuAI 預設設定設計的格式。",
