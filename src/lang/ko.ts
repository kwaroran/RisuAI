export const languageKorean = {
  ...
  help: {
    ...
    jailbreak:
      "jailbreak 프롬프트는 jailbreak 토글이 켜져있을 때 작동되는 프롬프트입니다.",
    globalNote: "모델에 강력한 영향을 주는 프롬프트입니다. UJB라고도 합니다.",
    autoSuggest:
      "자동으로 유저의 응답을 제안할 때 선택지를 생성하기 위해 사용되는 프롬프트입니다.",
    formatOrder:
      "프롬프트의 배치 순서입니다. 아래쪽에 있을 수록 더 큰 영향을 줍니다.",
    ...
    chatNote:
      "현재 채팅에 포함되는 메모리 또는 UJB라고도 알려진 프롬프트입니다.",
    personality:
    ...
    loreSelective:
      "선택 모드가 토글되면 활성화 키와 보조 키 모두 일치해야 로어가 활성화됩니다.",
    ...
    inlayImages:
      "활성화하면 이미지를 채팅에 삽입할 수 있으며, 지원하는 경우 AI가 볼 수 있습니다.",
    metrica:
    ...
    lorePlus:
      "LoreBook+는 더 나은 매칭 성능을 위해 벡터DB를 사용하는 실험적인 기능입니다.",
    ...
    sayNothing:
      "활성화되면 문자열이 입력되지 않을 때 '아무 말도 하지 않음'을 입력합니다.",
    showUnrecommended:
    ...
    forceProxyAsOpenAI:
      "활성화되면 커스텀 모델 사용 시 OpenAI 형식을 강제로 사용합니다.",
    forcePlainFetch:
      "활성화되면 네이티브 HTTP 요청 대신 브라우저 Fetch API를 사용합니다. 이로 인해 CORS 오류가 발생할 수 있습니다.",
    autoFillRequestURL:
      "활성화되면 요청 URL을 현재 모델과 일치하도록 자동으로 채웁니다.",
    chainOfThought: "활성화되면 생각의 사슬 프롬프트를 프롬프트에 추가합니다.",
    gptVisionQuality:
      "이 옵션은 이미지 감지 모델의 품질을 설정하는 데 사용됩니다. 품질이 높을수록 감지가 더 정확하지만 토큰이 더 많이 사용됩니다.",
    genTimes:
    ...
    removePunctuationHypa:
      "활성화되면 HypaMemory를 실행하기 전에 구두점을 제거합니다.",
    additionalParams:
      '요청 본문에 추가될 매개변수입니다. 특정 매개변수를 제외하려면, 해당 값에 `{{none}}`을 입력하십시오. 요청 본문이 아닌 헤더를 추가하고 싶다면, `header::Authorization`과 같이 키 앞에 `header::`를 붙이십시오. 값을 JSON 형식으로 지정하려면, `json::{"key":"value"}`와 같이 값 앞에 `json::`를 붙이십시오. 그렇지 않은 경우, 값의 타입은 자동으로 결정됩니다.',
    antiClaudeOverload:
      "클로드 과부하가 발생하면, RisuAI는 동일한 프롬프트로 계속 진행하여 이를 방지하려고 시도하며, 과부하 발생 가능성을 줄입니다. 이 기능은 스트리밍 응답에만 작동합니다. 비공식 API 엔드포인트에서는 작동하지 않을 수 있습니다.",
    triggerScript:
      '트리거 스크립트는 조건이 충족될 때 실행되는 사용자 지정 스크립트입니다. 이는 채팅 데이터를 수정하고, 명령을 실행하고, 변수를 변경하는 등 다양한 용도로 사용될 수 있습니다. 타입은 트리거되는 시점에 따라 달라집니다. 또한, `{{button::Display::TriggerName}}`을 사용하여 버튼으로 실행하거나, `risu-trigger="<TriggerName>"` 속성을 가진 HTML 버튼으로 실행할 수도 있습니다.',
    autoContinueChat:
      "활성화하면, 구두점으로 끝나지 않는 채팅의 경우 계속 이어가려고 시도합니다. 구두점을 사용하지 않는 언어에서는 이 기능을 절대 사용하지 마십시오.",
    combineTranslation:
    ...
    dynamicAssets:
      "활성화하면, 데이터를 처리할 때 에셋 이름을 찾을 수 없는 경우, 벡터 검색을 사용하여 가장 유사한 에셋 이름을 찾아 그 이름으로 교체합니다.",
    dynamicAssetsEditDisplay:
      "활성화하면, 동적 에셋이 디스플레이 수정 단계에도 적용됩니다. 하지만 이로 인해 성능 문제가 발생할 수 있습니다.",
    nickname:
      "닉네임이 설정된 경우, 채팅에서 캐릭터의 이름 대신 {{char}} 또는 <char>에 사용됩니다.",
    useRegexLorebook:
      "활성화하면, 로어북 검색에 문자열 매칭 대신 정규식을 사용합니다. /regex/flags 형식을 사용합니다.",
    customChainOfThought:
      "경고: 생각의 사슬 토글은 더 이상 사용을 권장하지 않습니다. 대신 다른 프롬프트 항목에 생각의 사슬 프롬프트를 넣으세요.",
    customPromptTemplateToggle:
      "여기에서 자신만의 프롬프트 토글을 정의할 수 있습니다. `<toggle variable>=<toggle name>` 형식을 사용하고 줄바꿈으로 구분합니다. 예를 들어, `cot=Toggle COT`과 같이 사용할 수 있습니다. 프롬프트에서 `{{getglobalvar::toggle_<toggle variable>}}`를 사용하여 이러한 토글을 사용할 수 있습니다. 예: `{{getglobalvar::toggle_cot}}`.",
    defaultVariables:
    ...
    lowLevelAccess:
      "활성화하면, 높은 컴퓨팅 성능을 요구하고 캐릭터 내 트리거를 통해 AI 모델을 실행하는 기능에 대한 접근을 활성화합니다. 이러한 기능이 정말로 필요하지 않은 경우 활성화하지 마세요.",
    triggerLLMPrompt:
      "모델로 전송될 프롬프트입니다. `@@role user`, `@@role system`, `@@role assistant`를 사용하여 여러 턴과 역할을 지정할 수 있습니다. 예를 들면 다음과 같습니다: \n```\n@@role system\nrespond as hello\n@@role assistant\nhello\n@@role user\nhi\n```",
    legacyTranslation:
      "활성화하면, 번역 후 후처리하는 대신 번역 전에 마크다운과 따옴표를 전처리하는 구 번역 방법을 사용합니다.",
    luaHelp:
      "Lua 스크립트를 트리거 스크립트로 사용할 수 있습니다. 당신은 onInput, onOutput, onStart 함수를 정의할 수 있습니다. onInput은 사용자가 메시지를 보낼 때 호출되고, onOutput은 캐릭터가 메시지를 보낼 때 호출되며, onStart는 채팅이 시작될 때 호출됩니다. 자세한 내용은 문서를 참조하세요.",
    claudeCachingExperimental:
      "클로드의 캐싱은 모델 비용을 줄일 수 있는 실험적인 기능이지만, 리롤 없이 사용하면 비용이 증가할 수도 있습니다. 이는 실험적인 기능이므로 불안정할 수 있으며 향후 동작이 변경될 수 있습니다.",
    urllora:
      "당신은 모델 파일의 직접 다운로드 링크를 사용할 수 있습니다. https://sites.google.com/site/gdocs2direct/ 와 같은 웹사이트처럼 구글 드라이브에서 직접 URL을 만들거나, civitai URL을 사용하고 AIR(`urn:air:flux1:lora:civitai:180891@776656` 또는 그냥 `civitai:180891@776656` 처럼 보임)을 복사하여 붙여넣을 수 있습니다.",
    namespace:
      "네임스페이스는 모듈의 고유 식별자입니다. 모듈 간의 충돌을 방지하고 프리셋, 다른 모듈 등과의 상호 작용에 사용됩니다. 무엇을 넣어야 할지 확실하지 않으면 비워두세요.",
    moduleIntergration:
      "모듈 통합 섹션에 모듈 네임스페이스를 넣어 모듈을 활성화할 수 있습니다. 여러 모듈을 활성화하려면 쉼표로 구분할 수 있습니다. 예: `module1,module2,module3`. 이것은 프리셋에 따라 모듈 사용을 다양화하려는 고급 사용자를 위한 것입니다.",
    customCSS:
      "스타일링을 위한 커스텀 CSS입니다. 문제가 발생하면 (Ctrl + .)를 눌러 비활성화/활성화할 수도 있습니다.",
    betaMobileGUI:
      "활성화하면, 작은(800px 미만) 화면에서 베타 모바일 GUI를 사용합니다. 새로고침이 필요합니다.",
    unrecommended:
      "이것은 권장되지 않는 설정입니다. 이 설정을 사용하는 것은 권장되지 않습니다.",
    jsonSchema:
      '이것은 AI 모델이 JSON Schema를 지원하는 경우, AI 모델에게 전송될 JSON Schema입니다.\n\n하지만 JSON Schema는 배우기 어렵기 때문에, RisuAI에서는 JSON Schema 대신 TypeScript 인터페이스의 서브셋을 사용할 수 있습니다. RisuAI가 런타임에 이를 변환합니다. 예를 들어, 다음과 같은 JSON을 보내고 싶은 경우:\n\n```js\n{\n "name": "RisuAI", //name은 반드시 RisuAI여야 합니다,\n "age": 1, //age는 반드시 숫자여야 합니다,\n "icon": "slim", //icon은 반드시 \'slim\' 또는 \'rounded\'여야 합니다\n "thoughts": ["Good View!", "Lorem"] //thoughts는 반드시 문자열 배열이어야 합니다\n}\n```\n\n당신은 다음 TypeScript 인터페이스를 넣을 수 있습니다:\n\n```typescript\ninterface Schema {\n  name: string;\n  age: number;\n  icon: \'slim\'|\'rounded\'\n  thoughts: string[]\n}\n```\n\n인터페이스의 이름은 중요하지 않습니다. 더 자세한 정보는 타입스크립트 공식 문서를 확인하세요. (https://www.typescriptlang.org/docs/handbook/interfaces.html), 그리고 어떤 TypeScript 서브셋이 지원되는지 확인하려면 아래 내용을 참고하세요.<details><summary>지원되는 TypeScript 서브셋</summary>\n\n지원되는 타입은 `boolean`, `number`, `string`, `Array`입니다. 단위 타입, 교차 타입, 유니온 타입, 옵셔널, 리터럴 타입 등과 같은 고급 타이핑은 다음 경우를 제외하고는 지원되지 않습니다:\n\n        - 원시 타입의 배열: (예: `string[]`, `Array<boolean>`)\n        - 문자열 간의 단위 타입: (예: `\'slim\'|\'rounded\'`).\n\n        속성은 한 줄에 하나씩만 있어야 합니다. 한 줄에 여러 속성이 있으면 오류가 발생합니다. 속성과 인터페이스의 이름은 반드시 ASCII 범위 내의 라틴 문자여야만 합니다. 속성의 이름은 작은따옴표나 큰따옴표로 묶어서는 안 됩니다. 인터페이스 내부의 중첩은 지원되지 않습니다. 속성이 정의된 줄에는 `{` 또는 `}`를 넣을 수 없습니다. 더 고급 타입을 사용하고 싶다면, 대신 JSON Schema를 사용하세요.\n        </details>',
    strictJsonSchema:
      "활성화되면 일부 모델에서 JSON에 대해 제공된 스키마를 엄격하게 따릅니다. 비활성화되면 JSON 스키마를 무시할 수 있습니다.",
    extractJson:
      '비어 있지 않으면 응답에서 특정 JSON 데이터를 추출합니다. 예를 들어, 응답 `{"response": {"text": ["hello"]}}`에서 `response.text[0]`을 추출하려면 `response.text.0`을 입력하면 됩니다.',
    translatorNote:
      "여기에서 캐릭터마다 별도의 번역 프롬프트를 넣을 수 있습니다. 해당 옵션은  Ax. model 번역을 사용할 때만 적용됩니다. 언어 설정에서 `{{slot::tnote}}`를 넣으세요. 그룹챗에서는 작동하지 않습니다.",
    groupInnerFormat:
      "이것은 화자가 아닌 그룹 채팅의 캐릭터에 사용되는 포맷을 정의합니다. 비어 있지 않으면 기본 포맷 대신 이 포맷을 사용합니다. `Group Other Bot Role`이 `assistant`인 경우 화자에게도 적용됩니다.",
    groupOtherBotRole:
      "이것은 화자가 아닌 그룹 채팅의 캐릭터에 사용되는 역할을 정의합니다.",
    chatHTML:
      "각 채팅에 삽입될 HTML입니다.\n\nCBS 및 특수 태그를 사용할 수 있습니다.\n- `<risutextbox>`: 텍스트 렌더링에 사용될 텍스트 상자\n- `<risuicon>`: 사용자 또는 어시스턴트 아이콘\n- `<risubuttons>`: 채팅 편집, 번역 등을 위한 아이콘 버튼\n- `<risugeninfo>`: 생성 정보 버튼.",
    ...
    v2RegexTest:
      "정규표현식에 매칭이 되면 1, 매칭이 되지 않으면 0을 출력합니다.",
    v2Calculate:
  ...
  setup: {
    ...
    openAIProvider:
      "OpenAI GPT는 가장 좋은 모델이지만, 유료고 검열이 있습니다.",
  },
  triggerCategories: {
    Special: "특수",
    Control: "제어",
  ...
  donatorPatreonDesc:
    "기본적으로 개인 정보 보호를 위해 목록에 표시되지 않습니다. 닉네임을 표시하려면 RisuAI의 Patreon 페이지로 이동하여 링크 버튼을 누르세요.",
  ...
  shareExport: "공유/엑스포트",
  risupresetDesc:
    "Risupreset 포맷은 포맷을 공유하기 위한 RisuAI 전용 포맷입니다.",
  risuMDesc: "RisuM 포맷은 RisuAI 모듈 전용으로 설계된 포맷입니다.",
  ...
  hypaChunkSize: "청크 크기",
  hypaV2Desc:
    "하이파메모리 V2는 벡터 검색과 요약을 둘 다 사용하는 장기기억 시스템입니다.",
  ...
  otherUserRequesting:
    "다른 사용자가 이미 요청 중입니다. 나중에 다시 시도하세요.",
  jsonSchema: "JSON 스키마",
