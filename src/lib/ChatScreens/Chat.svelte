<script lang="ts">
    ...
    import { runLuaButtonTrigger } from "src/ts/process/script/trigger";
    ...
    async function rm(e: MouseEvent, rec?: boolean) {
        ...
        const rm = DBState.db.askRemoval
            ? await alertConfirm(language.removeChat)
            : true;
        if (rm) {
    ...
    let blankMessage = $state(
        (message === "{{none}}" || message === "{{blank}}" || message === "") &&
            idx === -1,
    );
    $effect.pre(() => {
        blankMessage =
...
{#snippet icons(options: { applyTextColors?: boolean } = {})}
    ...
    >
        ...
        {#if DBState.db.useChatCopy && !blankMessage}
            <button
                class="ml-2 hover:text-blue-500 transition-colors button-icon-copy"
                onclick={async () => {
                    if (window.navigator.clipboard.write) {
                        try {
                            ...
                            const modelInfo = messageGenerationInfo
                                ? capitalize(
                                      getModelInfo(messageGenerationInfo.model)
                                          .shortName,
                                  )
                                : isUserMessage
                                  ? "User"
    ...
    <div style="padding: 20px;">
        ...
        <div style="text-align: center; margin-top: 1rem; padding-top: 0.75rem; border-top: 1px solid ${root.style.getPropertyValue("--risu-theme-darkborderc")};">
            <span style="font-size: 0.75rem; color: ${root.style.getPropertyValue("--risu-theme-textcolor2")}; opacity: 0.7;">From RisuAI</span>
                            ...
                            await window.navigator.clipboard.write([
                                new ClipboardItem({
                                    "text/plain": new Blob([msgDisplay], {
                                        type: "text/plain",
                                    }),
        ...
        {#if idx > -1}
            {#if DBState.db.characters[selIdState.selId].type !== "group" && DBState.db.characters[selIdState.selId].ttsMode !== "none" && DBState.db.characters[selIdState.selId].ttsMode}
                <button
                    class="ml-2 hover:text-blue-500 transition-colors button-icon-tts"
                    onclick={() => {
            ...
            {#if !$ConnectionOpenStore}
                ...
                <button
                    class="ml-2 hover:text-blue-500 transition-colors button-icon-remove"
        ...
        {#if DBState.db.translator !== "" && !blankMessage}
            <button
                class={"ml-2 cursor-pointer hover:text-blue-500 transition-colors button-icon-translate " +
        ...
        {#if rerollIcon || altGreeting}
            {#if DBState.db.swipe || altGreeting}
                <button
                    class="ml-2 hover:text-blue-500 transition-colors button-icon-unreroll"
                ...
                {#if firstMessage && DBState.db.swipe && DBState.db.showFirstMessagePages}
                    <span class="ml-2 text-xs text-textcolor2"
                        >{currentPage}/{totalPages}</span
                    >
                {/if}
                <button
                    class="ml-2 hover:text-blue-500 transition-colors button-icon-reroll"
                    class:dyna-icon={rerollIcon === "dynamic"}
                    onclick={onReroll}
                >
                    <ArrowRight size={22} />
...
{#snippet renderGuiHtmlPart(dom: HTMLElement)}
    ...
    {:else if dom.tagName === "A"}
        <a
            ...
            dom.getAttribute("href").startsWith("https")
...
>
    <div
        class="text-textcolor mt-1 ml-4 mr-4 mb-1 p-2 bg-transparent flex-grow border-t-gray-900 border-opacity-30 border-transparent flexium items-start max-w-full"
    >
        {#if DBState.db.theme === "mobilechat" && !blankMessage}
            <div
                class={role === "user"
                    ? "flex items-start w-full justify-end"
                    : "flex items-start"}
            >
                {#if role !== "user"}
        ...
        {:else if DBState.db.theme === "cardboard" && !blankMessage}
            <div class="w-full flex flex-col px-0 sm:px-4 py-4 relative">
                <div
                    class="bg-gradient-to-b from-gray-100 to-gray-200 rounded-lg shadow-lg border-gray-400 border p-4 flex flex-col"
                >
                    <div class="flex gap-4 mt-2 flex-col sm:flex-row">
                        ...
                        {#if editMode}
                            <textarea
                                class="flex-grow h-138 sm:h-96 overflow-y-auto bg-transparent text-black p-2 mb-2 resize-none message-edit-area"
                                bind:value={message}
                            ></textarea>
                        {:else}
                            <div
                                class="flex-grow h-138 sm:h-96 overflow-y-auto p-2 mb-2 sm:mb-0"
                            >
                                {@render textBox()}
                            </div>
                        {/if}
                ...
                <div
                    class="absolute bottom-0 right-0 bg-gradient-to-b from-gray-200 to-gray-300 p-2 rounded-md border border-gray-400 text-gray-400"
                >
        ...
        {:else}
            ...
            <span
                class="flex flex-col ml-4 w-full max-w-full min-w-0 text-black"
            >
                <div class="flexium items-center chat-width">
                    {#if DBState.db.characters[selIdState.selId]?.chaId === "§playground" && !blankMessage}
                        <span
                            class="chat-width text-xl border-darkborderc flex items-center text-textcolor"
                        >
                            <span
                                >{DBState.db.characters[selIdState.selId].chats[
                                    DBState.db.characters[selIdState.selId]
                    ...
                    {:else if !blankMessage && !$HideIconStore}
                        <span class="chat-width text-xl unmargin text-textcolor"
                            >{name}</span
                        >
                    {/if}
                    {@render icons()}
