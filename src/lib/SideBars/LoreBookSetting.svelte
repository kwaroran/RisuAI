...
<div class="flex w-full">
  ...
  >
    <span
      >{$DataBase.characters[$selectedCharID].type === "group"
        ? language.group
...
{#if submenu !== 2}
  <span class="mb-6 mt-2 text-sm text-gray-500"
    >{submenu === 0
      ? $DataBase.characters[$selectedCharID].type === "group"
    ...
    {#if submenu === 0}
      ...
      {:else}
        {#each $DataBase.characters[$selectedCharID].globalLore as book, i}
          ...
          {/if}
          <LoreBookData
            bind:value={$DataBase.characters[$selectedCharID].globalLore[i]}
            onRemove={() => {
    ...
    {:else if submenu === 1}
      ...
      {:else}
        {#each $DataBase.characters[$selectedCharID].chats[$DataBase.characters[$selectedCharID].chatPage].localLore as book, i}
          ...
          {/if}
          <LoreBookData
            bind:value={$DataBase.characters[$selectedCharID].chats[
...
{:else if $DataBase.characters[$selectedCharID].loreSettings}
  ...
  <div class="mt-4 flex items-center">
    <Check
      bind:check={$DataBase.characters[$selectedCharID].loreSettings
        .recursiveScanning}
    />
  ...
  <input
    ...
    min={0}
    max="20"
    bind:value={$DataBase.characters[$selectedCharID].loreSettings.scanDepth}
  />
  ...
  <input
    ...
    min={0}
    max="4096"
    bind:value={$DataBase.characters[$selectedCharID].loreSettings.tokenBudget}
  />
{:else}
  <div class="mt-4 flex items-center">
    <Check
...
{#if submenu !== 2}
  <div class="mt-2 flex text-gray-500">
    ...
    <button
      ...
      class="ml-1 cursor-pointer hover:text-neutral-200"
    >
      <DownloadIcon />
    </button>
    <button
      ...
      class="ml-2 cursor-pointer hover:text-neutral-200"
    >
      <FolderUpIcon />
    </button>
  </div>
