...
<div class="absolute w-full h-full z-40 bg-black bg-opacity-50 flex justify-center items-center">
    <div class="bg-darkbg p-4 break-any rounded-md flex flex-col max-w-3xl w-72 max-h-full overflow-y-auto">
        <div class="flex items-center text-textcolor mb-4">
            <h2 id="chat-list-title" class="mt-0 mb-0">{language.chatList}</h2>
        ...
        {#each DBState.db.characters[$selectedCharID].chats as chat, i}
            <button 
                ...
                aria-label={`${chat.name} ${language.chat || "채팅"}`}
            >
                {#if editMode}
                    <TextInput bind:value={DBState.db.characters[$selectedCharID].chats[i].name} padding={false} ariaLabel={`${chat.name} ${language.name || "이름"}`}/>
                {:else}
                    <span>{chat.name}</span>
                {/if}
                <div class="flex-grow flex justify-end">
                    <div 
                        ...
                        aria-label={`${chat.name} ${language.export || "내보내기"}`}
                    >
                    ...
                    <div 
                        ...
                        aria-label={`${chat.name} ${language.delete || "삭제"}`}
                    >
                        <TrashIcon size={18}/>
                    </div>
                </div>
        ...
        <div class="flex mt-2 items-center">
            <button 
                ...
                aria-label={language.newChat || "새 채팅"}
            >
                <PlusIcon/>
            </button>
            <button 
                ...
                aria-label={language.import || "가져오기"}
            >
            ...
            <button 
                ...
                aria-label={editMode ? (language.viewMode || "보기 모드") : (language.editMode || "편집 모드")}
            >
                <EditIcon size={18}/>
            </button>
        </div>
