...
{#if openOptions}
    ...
    <div class="fixed top-0 w-full h-full left-0 bg-black bg-opacity-50 z-50 flex justify-center items-center" role="button" tabindex="0" onclick={() => {
        close()
    }} aria-label="Close model selection dialog">
        <div class="w-96 max-w-full max-h-full overflow-y-auto overflow-x-hidden bg-bgcolor p-4 flex flex-col" role="dialog" aria-modal="true" aria-labelledby="model-list-title" onclick={(e)=>{
            e.stopPropagation()
            onclick?.(e)
        }}>
