...
{#if sort !== 'random' && sort !== 'recommended'}
    <div class="w-full flex justify-center">
        <div class="flex">
            ...
            <button class="bg-darkbg h-14 w-14 min-w-14 rounded-lg ml-2 flex justify-center items-center transition-shadow" aria-label={`Current page ${page + 1}`}>
                <span>{page + 1}</span>
            </button>
    ...
    <div class="top-0 left-0 z-50 fixed w-full h-full bg-black bg-opacity-50 flex justify-center items-center" role="button" tabindex="0" onclick={() => {
        ...
        <div class="max-w-full bg-darkbg rounded-md flex flex-col gap-4 overflow-y-auto p-4">
            <h1 class="font-bold text-2xl w-full">
                ...
                <button class="float-right text-textcolor2 hover:text-green-500" onclick={() => {menuOpen = false}} aria-label="Close menu">
                    <XIcon />
                </button>
            </h1>
            ...
            })} aria-label="Import character from URL or ID">Import Character from URL or ID</button>
        </div>
    </div>
{/if}