    ...
    <div class="bg-darkbg rounded-md p-4 max-w-full flex flex-col w-2xl max-h-full overflow-y-auto" role="button" tabindex="0" onclick={(e)=>{
        ...
        {#if !char.realmId}
            <div class="flex items-center flex-wrap mt-4">
                <button class="bg-bgcolor p-2 rounded-lg" class:ring-1={!privateMode} onclick={() => {privateMode = false}} aria-label="Show author ID">🌏 Show Author ID</button>
                <button class="bg-bgcolor p-2 rounded-lg ml-2" class:ring-1={privateMode} onclick={() => {privateMode = true}} aria-label="Show anonymized">🔒 Anonymized</button>
            </div>
            <div class="flex items-center flex-wrap mt-2">
                <button class="bg-bgcolor p-2 rounded-lg" class:ring-1={!nsfwMode} onclick={() => {nsfwMode = false}} aria-label="Set as safe content">🎖️ Safe</button>
                <button class="bg-bgcolor p-2 rounded-lg ml-2" class:ring-1={nsfwMode} onclick={() => {nsfwMode = true}} aria-label="Set as adult content">🔞 NSFW</button>
            </div>
        {:else}
            <div class="flex items-center flex-wrap mt-2">
                <button class="bg-bgcolor p-2 rounded-lg" class:ring-1={!update} onclick={() => {nsfwMode = false}} aria-label="Update">🚀 Update</button>
                <button class="bg-bgcolor p-2 rounded-lg ml-2" class:ring-1={update} onclick={() => {nsfwMode = true}} aria-label="Upload newly">⭐ Upload Newly</button>
            </div>
        {/if}
        {#if nsfwMode}
