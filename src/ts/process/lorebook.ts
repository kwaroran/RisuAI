...
export function addLorebook(type: number) {
  ...
  if (type === 0) {
    ...
    db.characters[selectedID].chats[page].localLore.push({
      key: "",
      comment: `New Lore ${
        db.characters[selectedID].chats[page].localLore.length + 1
      }`,
      content: "",
      mode: "normal",
      insertorder: 100,
...
export async function importLoreBook(mode: "global" | "local") {
  ...
  let lore =
    mode === "global"
      ? db.characters[selectedID].globalLore
    ...
    if (importedlore.type === "risu" && importedlore.data) {
      ...
      for (const key in entries) {
        ...
        lore.push({
          ...
          comment:
            currentLore.comment.length < 1
              ? "Unnamed Imported Lore"
              : currentLore.comment,
