  ...
  try {
    ...
    for (const line of splitedJs) {
      if (line.startsWith("//@risu-name")) {
        ...
        if (provied === "") {
          alertError(
            'plugin name must be longer than "", did you put it correctly?'
          );
          return;
        }
        name = provied.trim();
      ...
      if (line.startsWith("//@risu-display-name")) {
        ...
        if (provied === "") {
          alertError(
            'plugin display name must be longer than "", did you put it correctly?'
          );
          return;
        }
        name = provied.trim();
        ...
        case "resProvider": {
          ...
          if (checkNullish(provres.success) || checkNullish(provres.content)) {
            providerRes = {
              success: false,
              content:
                "provider didn't respond 'success' or 'content' in response object",
            };
          } else if (typeof provres.content !== "string") {
            providerRes = {
              success: false,
              content:
                "provider didn't respond 'content' in response object in string",
            };
          } else {
            providerRes = {
        ...
        case "resTrans": {
          ...
          if (checkNullish(provres.success) || checkNullish(provres.content)) {
            translatorRes = {
              success: false,
              content:
                "plugin didn't respond 'success' or 'content' in response object",
            };
          } else if (typeof provres.content !== "string") {
            translatorRes = {
              success: false,
              content:
                "plugin didn't respond 'content' in response object in string",
            };
          } else {
            translatorRes = {
              success: !!provres.success,
