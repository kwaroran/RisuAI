...
export async function sendChat(chatProcessIndex = -1): Promise<boolean> {
  ...
  if (nowChatroom.type === "group") {
    if (chatProcessIndex === -1) {
      ...
      currentChar = findCharacterbyIdwithCache(
        nowChatroom.characters[chatProcessIndex]
      );
      if (!currentChar) {
        alertError(
          `cannot find character: ${nowChatroom.characters[chatProcessIndex]}`
        );
        return false;
      }
    }
  ...
  if (!currentChar.utilityBot) {
    const mainp =
      currentChar.systemPrompt.length > 3
        ? currentChar.systemPrompt
        : db.mainPrompt;
    ...
    unformated.main.push({
      ...
      content: replacePlaceholders(
        mainp +
          (db.additionalPrompt === "" || !db.promptPreprocess
            ? ""
            : `\n${db.additionalPrompt}`),
        currentChar.name
      ),
  ...
  if (currentChar.postHistoryInstructions !== "") {
    unformated.authorNote.push({
      role: "system",
      content: replacePlaceholders(
        currentChar.postHistoryInstructions,
        currentChar.name
      ),
    });
  }
  ...
  {
    let description = replacePlaceholders(
      (db.promptPreprocess ? db.descriptionPrefix : "") + currentChar.desc,
      currentChar.name
    );
  ...
  let currentTokens =
    (await tokenize(
      Object.keys(unformated)
        .map((key) => {
          return (unformated[key] as OpenAIChat[])
            .map((d) => {
              return d.content;
            })
            .join("\n\n");
  ...
  for (const msg of ms) {
    let formedChat = processScript(
      currentChar,
      replacePlaceholders(msg.data, currentChar.name),
      "editprocess"
    );
    if (nowChatroom.type === "group") {
      if (msg.saying && msg.role === "char") {
        formedChat = `${
          findCharacterbyIdwithCache(msg.saying).name
        }: ${formedChat}`;
  ...
  if (req.type === "fail") {
    ...
    const result2 = processScriptFull(
      currentChar,
      reformatContent(req.result),
      "editoutput"
    );
    result = result2.data;
