...
export async function checkUpdate() {
  try {
    ...
    const da = await fetch(
      "https://raw.githubusercontent.com/kwaroran/RisuAI-release/main/version.json"
    );
    //@ts-ignore
...
export function checkOldDomain() {
  ...
  if (isOldDomain) {
    ...
    if (db.language === "ko") {
      alertMd(
        "# 웹 버전 RisuAI의 도메인이 **risuai.xyz**로 변경되었습니다" +
      ...
      alertMd(
        "# Web version RisuAI's domain has transfered to **risuai.xyz**" +
