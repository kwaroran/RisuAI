...
export async function downloadFile(name: string, data: Uint8Array) {
  ...
  if (isTauri) {
    ...
    downloadURL(
      `data:png/image;base64,${Buffer.from(data).toString("base64")}`,
      name
    );
  }
}
      ...
      if (ind === -1) {
        ...
        try {
          const hasCache: boolean = (
            await (await fetch("/sw/check/" + encoded)).json()
          ).able;
          if (hasCache) {
...
export async function saveImage(data: Uint8Array, customId: string = "") {
  ...
  if (isTauri) {
    await writeBinaryFile(`assets/${id}.png`, data, {
  ...
  if (!loaded) {
    try {
      if (isTauri) {
        ...
        if (
          !(await exists("database/database.bin", {
            dir: BaseDirectory.AppData,
          }))
        ) {
        ...
        setDatabase(
          JSON.parse(
            Buffer.from(
              pako.inflate(
                Buffer.from(
                  await readBinaryFile("database/database.bin", {
                    dir: BaseDirectory.AppData,
                  })
                )
              )
        ...
        setDatabase(
          JSON.parse(
            Buffer.from(pako.inflate(Buffer.from(gotStorage))).toString("utf-8")
          )
        );
  ...
  if (isTauri) {
    if (db.requester === "new") {
      try {
        ...
        const res: string = await invoke("native_request", {
          url: url,
          body: body,
          header: header,
          method: method,
        ...
        if (!d.success) {
          ...
          if (arg.rawResponse) {
            ...
            addFetchLog(
              JSON.parse(Buffer.from(d.body, "base64").toString("utf-8")),
              true
            );
            return {
              ok: true,
      ...
      if (db.characters[i].emotionImages) {
        for (let i2 = 0; i2 < db.characters[i].emotionImages.length; i2++) {
          if (
            ...
            db.characters[i].emotionImages[i2][1] = checkParge(
